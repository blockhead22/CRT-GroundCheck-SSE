...
| `tool_name` | string | "file_write", "shell_exec", "git", "desktop_action" |
| `action` | string | Human-readable description |
| `target` | string | File path, URL, or command |
| `result` | string | "success", "error", "partial" |
| `reversible` | bool | Whether the action can be undone |
| `reverse_action` | string | How to undo (if reversible) |
| `details` | JSON | Additional metadata |
| `checkpoint_approved` | bool | Whether user approved via checkpoint |

### Functions

| Function | Description |
|----------|-------------|
| `create_receipt(tool_name, action, target, result, ...)` | Factory with auto UUID and timestamp |
| `log_receipt(receipt, thread_id)` | Persist to SQLite |
| `get_receipts(thread_id, limit=20)` | Most recent first |
...
When `checkpointMeta.diff_preview` is present, `ActionCard.tsx` renders a syntax-highlighted unified diff:
- Green lines: additions
- Red lines: removals
- Blue lines: hunk headers

Shown above Yes/No buttons for user confirmation before write.

### Command Preview

Shell and git commands shown as `$ command` in a code block before execution.

### FilePill Component

`FilePill.tsx` renders file paths as interactive inline chips:
- File/folder icon
- Shortened filename
- Full path tooltip on hover
- Click-to-copy
- Terracotta accent color matching the warm dark theme

Auto-detected in assistant messages via regex (Windows `D:\path\file` and Unix `/path/file` patterns).

### Composer Attachments

The + button in the composer opens an attachment menu:
- Add file path (native OS file picker)
- Add folder path (full path prompt for absolute paths)
- Working directory shortcut
- Type path manually

Attached paths appear as removable pills and are prepended as `[file: path]` / `[dir: path]` when the message is sent.

---

## Tool Context Carry-Forward

When a follow-up message arrives within 120 seconds of a completed tool task, the recent tool output is injected into the generation context. This prevents the LLM from hallucinating answers to questions like "what was in that file?" — it sees the actual content.
...
|--------|------|-------------|
| GET | `/api/action-receipts` | List action receipts for a thread |
| GET | `/api/files/read` | Read a file |
| GET | `/api/files/list` | List a directory |
| GET | `/api/project/scan` | Scan a project |
| GET/PUT | `/api/settings/allowed-paths` | Manage allowed paths |
