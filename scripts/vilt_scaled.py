"""
VILT Scaled: Verification-In-the-Loop Training on 1B+ Models
=============================================================

Same VILT technique proven on SmolLM-135M (88% accuracy), now applied
to Qwen2.5-1.5B — a 1.54B parameter model. Uses LoRA adapters so
only 4.4M parameters (0.28%) are trained.

Architecture:
  1. Forward pass → supervised loss (teacher forcing on target answer)
  2. Generate freely → pipe through GroundCheck.verify() against fact ledger
  3. If contradictions found → loss *= (1 + penalty * trust_score)
  4. Brevity penalty prevents adversarial shortening
  5. Backprop amplified loss through LoRA adapters only

Requirements:
  - torch (CUDA), transformers, peft, groundcheck
  - ~5-6 GB VRAM during training (fits RTX 3060 12GB easily)

Usage:
  python scripts/vilt_scaled.py
  python scripts/vilt_scaled.py --model Qwen/Qwen2.5-1.5B
  python scripts/vilt_scaled.py --model HuggingFaceTB/SmolLM2-1.7B
"""
...
# ==============================================================
#  DATA LOADING — shared JSON files
# ==============================================================
...
# Training examples — plain-text targets (same as pretrained script).
TRAINING_EXAMPLES = [
...
class VILTConfig:
    ...
    learning_rate: float = 1e-4       # slightly lower for larger model
    num_steps: int = 200
    log_every: int = 10
    eval_every: int = 25
    ...
    early_stop_acc: float = 0.88      # higher bar for bigger model
...
class VILTScaledTrainer:
    """VILT trainer for HuggingFace pretrained models with LoRA."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="VILT Scaled — 1B+ model training")
    ...
    # ── train ──
    print(f"\n{'='*70}")
    print(f"  VILT TRAINING: {cfg.num_steps} steps   LR={cfg.learning_rate}   LoRA r={cfg.lora_r}")
    ...
    print(f"  Early stop: acc>={cfg.early_stop_acc:.0%} AND gc>={cfg.early_stop_gc:.0%}")
    print(f"{'='*70}")
    ...
    for step in range(1, cfg.num_steps + 1):
        ...
        if step % cfg.log_every == 0:
            ...
            if device == "cuda":
                ...
                vram_str = f"  VRAM={vram_now:.1f}GB"
            else:
                vram_str = ""

            print(f"\n  Step {step:3d}  ({el:.0f}s) [{phase}]{vram_str}")
            print(f"    sup={sl:.4f}  cs={cs:.3f}  mult={ml:.3f}  vilt={vl:.4f}")
            print(f"    GC pass={gp:.0%}  brevity={bp:.3f}  avg_tok={rt:.0f}")
        ...
        if step % cfg.eval_every == 0:
            ...
            if ev["accuracy"] > best_acc:
                ...
                print(f"    ** New best! acc={best_acc:.0%} saved to {best_dir.name}/")
    ...
    for i in range(0, len(logs), 10):
        chunk = logs[i:i+10]
        gc_timeline.append(sum(1 for x in chunk if x["gc_pass"]) / len(chunk))
        cs_timeline.append(sum(x["cs"] for x in chunk) / len(chunk))
    ...
    metrics = {
        ...
        "config": {
            "cw": cfg.contradiction_weight,
            "brevity_weight": cfg.brevity_weight,
            "curriculum_switch_step": cfg.curriculum_switch_step,
        },
    }
    mp = out_dir / "vilt_metrics.json"
    with open(mp, "w") as f:
    ...
    print(f"\n  Done. {model_short}: {pre['accuracy']:.0%} -> {post['accuracy']:.0%} in {total_time/60:.1f} min")
