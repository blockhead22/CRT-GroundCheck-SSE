...
def run_cell(level: str, mode: str, trial: int) -> dict:
...
def _load_existing_cells() -> tuple[list[dict], set[tuple[str, str, int]]]:
    ...
    with open(CSV_OUT, "r", newline="", encoding="utf-8") as f:
        for r in csv.DictReader(f):
            rows.append(r)
            try:
                keys.add((r["level"], r["mode"], int(r["trial"])))
...
def run_grid(levels: list[str], modes: list[str], trials: int,
    ...
    resume=True keeps the existing CSV and skips any (level, mode, trial) cell
    already present in it. resume=False starts fresh (deletes prior CSV/JSON).
    """
    ...
    total = len(levels) * len(modes) * trials
    i = 0
    t0 = time.time()
    for level in levels:
        for mode in modes:
            for trial in range(trials):
                i += 1
                if (level, mode, trial) in done:
                    print(
                        f"\n[GRID {i}/{total}] SKIP (already in CSV): "
                        f"level={level} mode={mode} trial={trial}",
                        flush=True,
                    )
                    continue
                print(
                    f"\n[GRID {i}/{total}] level={level} mode={mode} trial={trial} "
                    f"elapsed={time.time()-t0:.0f}s",
                    flush=True,
                )
                rec = run_cell(level, mode, trial)
    ...
    for level in levels:
        for mode in modes:
            cells = [r for r in all_records if r["level"] == level and r["mode"] == mode]
            n_esc = sum(1 for r in cells if r["escaped"])
            ete = [r["epochs_to_escape"] for r in cells if r["epochs_to_escape"] is not None]
            avg_ete = (sum(ete) / len(ete)) if ete else None
            print(
                f"  {level:20s} {mode:8s}  escape={n_esc}/{len(cells)}  "
                f"avg_ete={avg_ete}",
                flush=True,
            )
    return all_records
...
def main() -> int:
    ...
    if not (args.run or args.plot):
        ap.error("pass --run and/or --plot")

    if args.run:
        run_grid(args.levels, args.modes, args.trials, resume=args.resume)
    if args.plot:
