...
def run_orchestrator(
    ...
    orch_msg = str(req.message or "")

    runtime.safe_print(f"[ORCHESTRATOR] >>> ENTERING agent loop path (intent={getattr(task_intent, 'intent_type', '')!r}, reason={routing_reason})")
    runtime.ensure_governed_task(objective=orch_msg, max_iterations=10)
    runtime.update_governed_task(status=GovernedTaskStatus.RUNNING.value, wait_kind=None, question=None)
    runtime.append_governed_event("agent_loop_start", orch_msg, {"mode": "orchestrator", "reason": routing_reason})
    yield runtime.emit({"type": "agent_loop_start", "content": "Agent loop started", "metadata": {"mode": "orchestrator", "reason": routing_reason}})

    msg_lower = req.message.lower().strip()
    if any(word in msg_lower for word in ("read", "open", "look at", "check", "show", "what", "find")):
        ack_text = "Looking at that..."
    elif any(word in msg_lower for word in ("write", "edit", "update", "change", "fix", "patch")):
        ack_text = "On it."
    elif any(word in msg_lower for word in ("search", "research", "fetch", "web")):
        ack_text = "On it, looking that up..."
    elif any(word in msg_lower for word in ("run", "execute", "build", "test")):
        ack_text = "Running that..."
    else:
        ack_text = "On it."
    ...
    try:
        ...
        orch_gen_mode = str(_auth_orch.get_user_setting(uid_orch, "generation_mode", "cloud_claude") or "cloud_claude").strip()
        if orch_gen_mode == "cloud_openai":
            orch_brain = OpenAIBrain(model="gpt-4o")
        else:
            orch_brain = ClaudeCliBrain()
        runtime.safe_print(f"[ORCHESTRATOR] Brain selected: {orch_gen_mode} -> {getattr(orch_brain, '_model', 'unknown')}")
        ...
        try:
            ...
            orch_history = [f"{'User' if msg['role'] == 'user' else 'Aether'}: {msg['content'][:150]}" for msg in orch_recent]
            if orch_history:
        ...
        orch_gen = orch.run(
            orch_msg,
            conversation_history=orch_history or None,
            ...
            if etype == "plan":
                ...
                if est_depth is not None:
                    try:
                        ...
                        runtime.safe_print(f"[ORCHESTRATOR] Adaptive depth: plan declared estimated_depth={est_depth} → max_iterations={clamped}")
                ...
                yield runtime.emit({"type": "agent_thinking_token", "content": orch_event.get("content", ""), "metadata": {"step": "orchestrator_thinking", "alignment": orch_event.get("alignment")}})
                ...
                yield runtime.emit(
                    {
                        "type": "epistemic_event",
                        "content": orch_event.get("content", ""),
                        "metadata": {"event": "contradiction", "step_a": orch_event.get("step_a"), "step_b": orch_event.get("step_b")},
                    }
                )
                ...
                if tool_reasoning:
                    yield runtime.emit({"type": "agent_thinking_token", "content": tool_reasoning[:200], "metadata": {"step": "tool_loop", "alignment": tool_align}})
                yield runtime.emit({"type": "tool_start", "content": f"Running {tool_name}...", "metadata": {"tool_name": tool_name, "input": tool_args, "alignment": tool_align, "reasoning": tool_reasoning}})
                yield runtime.emit({"type": "tool_result", "content": tool_result[:500], "metadata": {"tool_name": tool_name, "status": tool_status, "step_index": len(orch_steps), "duration_ms": tool_ms, "alignment": tool_align}})
                orch_steps.append({"tool": tool_name, "args": tool_args, "status": tool_status})
                runtime.update_governed_task(steps_done=list(orch_steps), orch_answer_so_far=orch_answer)
                runtime.append_governed_event("tool_result", tool_result[:500], {"tool_name": tool_name, "status": tool_status, "step_index": len(orch_steps) - 1, "duration_ms": tool_ms})
            elif etype == "spawn_thinking":
                yield runtime.emit({"type": "agent_thinking_token", "content": orch_event.get("content", ""), "metadata": {"step": "spawn_agent", "subagent_task": orch_event.get("subagent_task", "")}})
            elif etype == "spawn_tool":
                st_name = orch_event.get("tool", "")
                st_status = orch_event.get("status", "ok")
                st_ms = orch_event.get("latency_ms")
                yield runtime.emit({"type": "tool_start", "content": f"[subagent] {st_name}", "metadata": {"tool_name": st_name, "input": orch_event.get("args", {}), "is_subagent": True, "subagent_task": orch_event.get("subagent_task", "")}})
                yield runtime.emit({"type": "tool_result", "content": orch_event.get("result", "")[:300], "metadata": {"tool_name": st_name, "status": st_status, "step_index": len(orch_steps), "duration_ms": st_ms, "is_subagent": True}})
                orch_steps.append({"tool": f"[subagent] {st_name}", "args": orch_event.get("args", {}), "status": st_status})
                runtime.update_governed_task(status=GovernedTaskStatus.AWAITING_SUBTASK.value, wait_kind=GovernedTaskWaitKind.SUBAGENT.value, steps_done=list(orch_steps), orch_answer_so_far=orch_answer)
                runtime.append_governed_event("tool_result", orch_event.get("result", "")[:300], {"tool_name": st_name, "status": st_status, "is_subagent": True})
            elif etype == "spawn_complete":
                spawn_success = orch_event.get("success", False)
                spawn_ms = orch_event.get("elapsed_ms", 0)
                runtime.update_governed_task(status=GovernedTaskStatus.RUNNING.value, wait_kind=None, steps_done=list(orch_steps), orch_answer_so_far=orch_answer)
                runtime.safe_print(f"[ORCHESTRATOR] Subagent complete: success={spawn_success}, {spawn_ms:.0f}ms")
                yield runtime.emit({"type": "status", "content": f"Subagent {'done' if spawn_success else 'failed'} ({spawn_ms:.0f}ms)", "metadata": {"is_subagent": True}})
                ...
                if checkpoint_tier == "file_write":
                    diff_suspended = {
                        "type": "diff_write",
                        "objective": orch_msg,
                        "steps_done": list(orch_steps),
                        "orch_answer_so_far": orch_answer,
                    ...
                    runtime.safe_print(f"[ORCHESTRATOR] Diff checkpoint suspended: {orch_meta.get('target_path')}")
                    yield runtime.emit({"type": "done", "content": orch_answer, "metadata": {"loop_suspended": True, "loop_question": checkpoint_content, "response_type": "diff_preview", "checkpoint_tier": "file_write"}})
                ...
                runtime.update_governed_task(orch_answer_so_far=orch_answer, steps_done=list(orch_steps))
                yield runtime.emit({"type": "token", "content": orch_answer})
            elif etype == "followup_suggest":
                followups = orch_event.get("followups", [])
                is_complete = orch_event.get("complete", True)
                if followups:
                    if is_complete:
                        runtime.update_governed_task(pending_followups=list(followups))
                    else:
                        runtime.update_governed_task(
                            status=GovernedTaskStatus.NEEDS_FOLLOWUP.value,
                            pending_followups=list(followups),
                            steps_done=list(orch_steps),
                            orch_answer_so_far=orch_answer,
                        )
                    runtime.append_governed_event("followup_suggest", "Suggested follow-ups", {"followups": followups, "complete": is_complete})
                    yield runtime.emit({"type": "followup_suggest", "content": "Suggested follow-ups", "metadata": {"followups": followups, "complete": is_complete}})
            elif etype == "ask_user":
                ...
                runtime.append_governed_event("ask_user", ask_question, {"task_status": GovernedTaskStatus.AWAITING_USER.value})
                runtime.safe_print(f"[ORCHESTRATOR] Loop suspended — ask_user: {ask_question[:80]!r}")
                yield runtime.emit({"type": "done", "content": orch_answer, "metadata": {"loop_suspended": True, "loop_question": ask_question, "response_type": "ask_user", "session_id": getattr(session_db, 'session_id', None)}})
                return StreamTerminalResult(terminal=True, handled=True, metadata={"loop_suspended": True})
            elif etype == "done":
                pass

        try:
            session_db.record_query(thread_id=req.thread_id, query_text=orch_msg, response_text=orch_answer)
            ...
            if orch_db_path:
                ...
                if _Path_orch(orch_db_path).exists():
                    ...
                    for row in orch_cursor.fetchall():
                        tr_raw = str(row[3] or "")
                        yield runtime.emit({"type": "trust_shift", "metadata": {"memoryId": str(row[0]), "from": round(float(row[1] or 0), 3), "to": round(float(row[2] or 0), 3), "reason": trust_reason_map.get(tr_raw.lower().strip(), tr_raw), "text": ""}})
                    orch_conn.close()
        ...
        try:
            ...
            if orch_steps and len(orch_steps) >= 3:
                ...
                if ans_stripped.endswith("?") and len(ans_stripped) > 50:
                    outbox.push(thread_id=req.thread_id, content=ans_stripped.split("\n")[-1].strip(), trigger="agent_loop_followup", metadata={"steps": len(orch_steps), "source": "agent_loop"})
        ...
        try:
            ...
            if rl_recent:
                ...
                if rl_dc > 0:
                    yield runtime.emit({"type": "drift", "content": f"{rl_dc} drift event(s)", "metadata": {"drift_count": rl_dc, "intent_alignment": round(rl_conf, 3), "total_trust_delta": 0.0}})
        ...
        runtime.safe_print(f"[ORCHESTRATOR] Complete: {len(orch_steps)} steps, answer_len={len(orch_answer)}")
        if runtime.governed_task:
            runtime.governed_task = session_db.complete_governed_task(str(runtime.governed_task["task_id"]), orch_answer) or runtime.governed_task
            runtime.append_governed_event("done", orch_answer, {"task_status": GovernedTaskStatus.COMPLETED.value, "generation_source": "agent_loop"})
        yield runtime.emit({"type": "agent_loop_complete", "content": orch_answer, "metadata": {"generation_source": "agent_loop"}})
        done_meta = {
            "tool_calls": orch_steps,
            "agent_loop": True,
            "orchestrator": True,
            "tools_executed": len(orch_steps) > 0,
            "response_type": "task",
            "gates_passed": True,
            "generation_source": "agent_loop",
        }
        yield runtime.emit({"type": "done", "content": orch_answer, "metadata": done_meta})
        return StreamTerminalResult(terminal=True, handled=True, metadata=done_meta)
    except Exception as orch_err:
        if runtime.governed_task:
            runtime.governed_task = session_db.fail_governed_task(str(runtime.governed_task["task_id"]), str(orch_err)) or runtime.governed_task
            runtime.append_governed_event("error", str(orch_err), {"task_status": GovernedTaskStatus.FAILED.value})
