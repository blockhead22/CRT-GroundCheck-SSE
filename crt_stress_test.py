...
print(f"\n⚠️  CONTRADICTIONS INTRODUCED: {len(metrics['contradictions_introduced'])}")
...
if detection_rate < 0.7:
    print(f"   ⚠️  LOW - Consider lowering theta_contra (currently 0.35)")
    print(f"   → Suggested: theta_contra = 0.25-0.30")
elif detection_rate > 0.95:
    print(f"   ⚠️  TOO SENSITIVE - May be detecting false positives")
...
if gate_pass_rate < 0.5:
    print(f"   ⚠️  LOW - Gates too strict, blocking legitimate queries")
    ...
    print(f"   ⚠️  HIGH - Gates may be too permissive")
...
if metrics['trust_scores']:
    ...
    if trust_variance < 0.1:
        print(f"   ⚠️  LOW - Trust scores not evolving enough")
...
if avg_conf < 0.6:
    print(f"   ⚠️  LOW - System may be too uncertain")
    print(f"   → Review confidence calculation in CRT core")
elif avg_conf > 0.9:
    print(f"   ⚠️  HIGH - System may be overconfident")
...
if metrics['memory_failures']:
    print(f"   ⚠️  {len(metrics['memory_failures'])} failures detected")
