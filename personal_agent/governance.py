"""
Governance Layer — Wrapper for CRT Immune Agents

Option D: Clean separation. The immune agents never touch CRT internals.
They inspect inputs and outputs at boundaries. The agents don't rewrite
the pipeline. They watch it and intervene at boundaries.

Architecture:
    govern_response()     → TemplateDetector → (if flagged) GapAuditor
    govern_memory_write() → SpeechLeakDetector → (if overwrite) MemoryCorruptionGuard
    govern_resolution()   → PrematureResolutionGuard
...
All five agents always available, but lazy evaluation:
    - TemplateDetector and SpeechLeakDetector are cheap (regex + cosine sim)
    - GapAuditor only runs if one of the first two flags something
    - PrematureResolutionGuard only fires on resolution attempts
    - MemoryCorruptionGuard only fires on overwrites
"""
...
class GovernanceLayer:
    ...
    Never touches CRT internals. Observe and flag only.
    ...
    def govern_response(
        ...
        if tier == GovernanceTier.HEDGE and gap_verdict:
            ...
            confidence_adj = -0.3  # template lock penalty
