"""Structural Tension Meter: Zero-LLM belief verification for CRT.

Measures the tension between two memories using only structural signals:
- Slot extraction (via fact_slots.extract_fact_slots_contextual)
- Embedding similarity (via embeddings.encode_text)
...
Integration points:
1. Memory write path (crt_memory.store_memory) — pre-storage tension check
2. Memory consolidation (memory_consolidation) — NLI pre-filter
3. Heartbeat breathing loop (heartbeat_executor) — continuous monitoring
"""
...
class StructuralTensionMeter:
    ...
    def __init__(self, encoder=None):
        """
        Args:
            encoder: Optional EmbeddingEngine instance. If None, lazy-loads
                     from embeddings.get_encoder() on first use.
        """
    ...
    def _extract_slots(self, text: str) -> Dict:
        ...
        Tries the production fact_slots extractor first (designed for first-person
        input like "I live in X"). Falls back to lightweight regex patterns for
        third-person memory format like "Nick lives in X".
        """
        slots: Dict = {}

        # Try production extractor first
        try:
            from .fact_slots import extract_fact_slots_contextual
    ...
    def _extract_slots_fallback(self, text: str) -> Dict:
        ...
        slots = {}
        _FallbackFact = type("FallbackFact", (), {
            "__init__": lambda self, slot, value, normalized, temporal_status="active":
                (setattr(self, "slot", slot), setattr(self, "value", value),
                 setattr(self, "normalized", normalized),
                 setattr(self, "temporal_status", temporal_status)),
        })
