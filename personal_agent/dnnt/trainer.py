"""DNNT trainer - triple-loss training loop with vocab expansion."""
...
class ReasoningTrainer:
    ...
    def __init__(
        ...
        model: Optional[DNNTMicroTransformer] = None,
    ):
        self.config = config
        self.tokenizer = tokenizer
        ...
        if model is None:
            model_config = DNNTConfig(vocab_size=tokenizer.vocab_size)
            model = DNNTMicroTransformer(model_config)

        # Dynamic vocab expansion from training corpus.
        all_examples = list(train_examples) + (list(val_examples) if val_examples else [])
        if all_examples:
            training_texts = [ex.to_training_format() for ex in all_examples]
            expand_model_vocab(model, tokenizer, training_texts)
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> Tuple[float, Dict[str, float]]:
        ...
        breakdown = {}
        if hasattr(self.model, "get_last_loss_breakdown"):
            breakdown = self.model.get_last_loss_breakdown()
    ...
    def train(self):
        ...
        while self.global_step < self.config.max_steps:
            ...
            for batch in self.train_loader:
                loss, breakdown = self.train_step(batch)
                running_loss += loss
                num_batches += 1
                self.global_step += 1
                ...
                if self.global_step % self.config.log_every == 0:
                    ...
                    if breakdown:
                        print(
                            "         | sem={semantic_loss:.4f} syn={syntactic_loss:.4f} "
                            "log={logical_loss:.4f} red={redundancy_loss:.4f}".format(**breakdown)
                        )
                ...
                if self.global_step % self.config.save_every == 0:
                    val_loss_for_best = self.best_val_loss
                    if self.val_dataset and self.val_losses:
                        val_loss_for_best = self.val_losses[-1][1]
                    ckpt_path = Path(self.config.output_dir) / f'checkpoint_{self.global_step}.pt'
                    self.save_checkpoint(str(ckpt_path), is_best=(self.val_dataset and val_loss_for_best == self.best_val_loss))
